...
module pwm_peripheral (
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
         pwm_signal <= 1'b0;
         end else begin pwm_signal <= (pwm_duty_cycle == 8'hFF) ? 1'b1 : (pwm_counter < pwm_duty_cycle);
        end
    end
    ...
    assign out[7:0] = en_reg_out_7_0 & ((|en_reg_pwm_7_0) ? {8{pwm_signal}} : 8'hFF); 

   assign out[15:8] = en_reg_out_15_8 & ((|en_reg_pwm_15_8) ? {8{pwm_signal}} : 8'hFF); 
